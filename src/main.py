...
@app.get("/", response_class=HTMLResponse, summary="Serve Web Test Interface")
def get_web_interface():
    """Serves the local developer HTML web interface."""
    template_path = Path(__file__).parent / "templates" / "index.html"
    if not template_path.exists():
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="Web interface template file not found.",
        )
...
@app.get("/api/v1/download/{file_type}", summary="Download output files")
def download_file(file_type: str):
    """
    Serves output files generated by the pipeline.
    Valid file types: transcript, merged, whisper, diarization, benchmark, metadata
    """
    file_map = {
        "transcript": ("transcript.txt", "text/plain"),
        "merged": ("merged.json", "application/json"),
        "whisper": ("raw_whisper.json", "application/json"),
        "diarization": ("raw_diarization.json", "application/json"),
        "benchmark": ("benchmark.json", "application/json"),
        "metadata": ("audio_metadata.json", "application/json"),
    }
    if file_type not in file_map:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=f"Invalid file type. Available: {list(file_map.keys())}",
        )
    ...
    if not file_path.exists():
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail=f"File {filename} not found. Please run the pipeline first.",
        )
